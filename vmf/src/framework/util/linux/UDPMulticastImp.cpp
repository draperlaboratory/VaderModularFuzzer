...
void UDPMulticastImp::buildSocket(std::string address, int port)
{
    ...
    if (fd < 0) {
        LOG_ERROR << "socket() errno=" << strerror(errno);
    ...
    if (
       ...
       LOG_ERROR << "setsockopt errno=" <<strerror(errno);
    ...
    if (bind(fd, (struct sockaddr*) &addr, sizeof(addr)) < 0) {
        LOG_ERROR << "bind errno=" << strerror(errno);
    ...
    if(status < 0)
    {
        LOG_ERROR << "fcntl errno=" <<strerror(errno);
...
int UDPMulticastImp::readData(char* msgbuf, int size)
{
    ...
    if (nbytes < 0) 
    {
        ...
        {   
            LOG_ERROR << "Socket error code:" << errno;
