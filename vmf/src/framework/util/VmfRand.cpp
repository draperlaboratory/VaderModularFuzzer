 ...
 */
unsigned long VmfRand::randBetween(unsigned long min, unsigned long max) {
 ...
 */
int VmfRand::randBetween(int min, int max) {
    return min + randBelow(max - min + 1);
}
 ...
 *
 * @param limit the upper bound
 * @return Random positive int value below upper bound
 */
...
/**
 * @brief Generate and return a random value lower than the limit (exclusive) and NOT equal to some value.
 *
 * @param limit the upper bound
 * @param exception the value that should be excluded
 * @return Random positive int value below upper bound
 */
