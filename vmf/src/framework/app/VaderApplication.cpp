...
/**
 * @brief Construct a new Vader Application
 * 
 */
VaderApplication::VaderApplication()
{
...
/**
 * @brief Destroy the Vader Application
 * 
 */
...
VaderApplication::~VaderApplication()
{
    ...
    {
        delete myConfig;
        //This will also delete all the modules constructed by the ConfigManager
    }
}


/**
 * @brief Initializes vader application, including parsing command line params
 * 
 * @param argc 
 * @param argv 
...
bool VaderApplication::init(int argc, char** argv)
{
...
bool VaderApplication::localInit()
{
    LOG_INFO << "Initializing VMF " << VERSION_NUMBER << " in standalone mode";

    //Read in the config file
    myConfig = new ConfigManager(configFiles);
    myConfig->readConfig();
...
bool VaderApplication::serverInit()
{
    ...
    this->myConfig = new ConfigManager(configFiles);
 ...
 */
bool VaderApplication::pollForTasking()
{
    bool valid = false;
 ...
 */
void VaderApplication::loadAndInitModules()
{
    ...
    std::string sortByKey  = myConfig->getStringParam(ConfigManager::STORAGE_MODULE_KEY,"sortByKey",  "FITNESS");
    std::string sortByType = myConfig->getStringParam(ConfigManager::STORAGE_MODULE_KEY,"sortByType", "FLOAT");
    std::string sortOrder  = myConfig->getStringParam(ConfigManager::STORAGE_MODULE_KEY,"sortOrder",  "DESCENDING");
    ...
    myConfig->registerModuleStorageNeeds(myRegistry, myMetadataRegistry);
    ...
    //Initialize storage
    Module* storageModule = myConfig->getStorageModule();
    ...
    Module* rootModule = myConfig->getRootModule();
 ...
 * or existing fuzzing).
 */
void VaderApplication::clearCurrentModules()
{
    //Shutdown all of the modules, then
    //delete all of the existing modules
    myConfig->shutdownModules(*myStorage);
    myConfig->resetModuleRegistry();
 ...
 */
std::string VaderApplication::createOutputDir()
{
    ...
    VaderUtil::createDirectory(outputBaseDir.c_str());
    VaderUtil::createDirectory(outputPath.c_str());
 ...
 */
void VaderApplication::loadPlugins()
{
    //Load anything in the plugins directory
    bool pluginsFound = false;
    std::string vmfDir = VaderUtil::getExecutablePath();
    std::string mainPlugins = vmfDir + "/plugins";
    if(VaderUtil::directoryExists(mainPlugins))
    {
 ...
 * @return false otherwise
 */
bool VaderApplication::parseConfigParams(int argc, char** argv)
{
...
/**
* @brief Helper method to print vader usage instructions
 * 
 */
void VaderApplication::printUsage()
{
    printf("   USAGE: vader -c <yaml config file> \n");
    printf("       (multiple -c options may be provided)\n");
    printf("   FOR DISTRIBUTED MODE: vader -d <yaml config file>\n");
    printf("       (in this case the config file must contain server config settings)\n");
}
 
/**
 * @brief Runs vader (runs infinitely until shutdown is called)
 * Note: other exception types may be thrown from third party libraries
 * 
 * @throws RuntimeException if any errors are encountered
 */
void VaderApplication::run()
{
    if(distributedMode)
    {
        runDistributed();
    ...
    LOG_INFO << "----SHUTTING DOWN VADER MODULES----";
    myConfig->shutdownModules(*myStorage);

    LOG_INFO << "----VADER TERMINATED----";
    Logging::shutdown();
}
 ...
 */
void VaderApplication::runStandalone()
{
    bool isFirstPass = true;

    if(nullptr == myController)
    {
        throw RuntimeException("Attempt to run uninitialized vader application", RuntimeException::USAGE_ERROR);
    }
 ...
 */
void VaderApplication::runDistributed()
{
    bool isFirstPass = true;
    ...
    while(!shutdownSignalReceived)
    {
        ...
        else
        {
            ...
            LOG_ERROR << "Invalid VaderApplication State:" << myState;
            throw RuntimeException("Invalid VaderApplicationState", RuntimeException::OTHER);
        }
 ...
 */
void VaderApplication::doRunningState(std::string& reason, bool performCorpusUpdate, bool isFirstPass)
{
    if(nullptr == myController)
    {
 ...
 * @param reason state transition reason used for communicating back to the server
 */
void VaderApplication::doRestartingState(std::string& reason)
{
    clearCurrentModules();
 ...
 * @param reason state transition reason used for communicating back to the server
 */
void VaderApplication::doIdleState(std::string& reason)
{
 ...
 * @param reason state transition reason used for communicating back to the server
 */
void VaderApplication::doTaskingState(std::string& reason)
{
    CDMSClient* client = CDMSClient::getInstance();
    try
    {
 ...
 * @param performCorpusUpdate when true, a corpus update should be performed
 */
void VaderApplication::handleCommands(std::string& reason, bool& performCorpusUpdate)
{
