 ...
 */
void GramatronSpliceMutator::mutateTestCase(StorageModule& storage, StorageEntry* baseEntry, StorageEntry* newEntry, int testCaseKey)
{
    ...
    splice(newEntry, baseEntry, storage, testCaseKey);
 ...
 * @param storage storage access need for retrieving a second test case
 * @param testCaseKey the field to write to
 */
void GramatronSpliceMutator::splice(StorageEntry* newEntry, StorageEntry* baseEntry, StorageModule& storage, int testCaseKey)
{
    ...
    int size = secondEntry->getBufferSize(autRepKey);

    if(size <= 0)
    {
        throw RuntimeException("GramatronSpliceMutator second test case has zero sized buffer", RuntimeException::USAGE_ERROR);
    }
