...
int CorpusMinimization::minimizeCorpus(StorageModule& storage)
{
    ...
    while(allEntries->hasNext())
    {
        ...
        //Allocate/Clear the corpusCoverage map (first test case only)
        //And make sure the size is correct for subsequent test cases
        if(firstPass)
        {
            unsigned int newMapSize = traceBitsSize;
            if(newMapSize != mapSize)
            {
                if(mapSize != 0) //The map size has changed since this module was last used, reallocate corpusCoverage
                {
                    delete corpusCoverage;
                }
                mapSize = newMapSize;
        ...
        else if (mapSize != (unsigned int) traceBitsSize)
        {
            throw RuntimeException("CorpusMinimization can only run when all test cases have the same sized map", 
                                    RuntimeException::USAGE_ERROR);
        }
