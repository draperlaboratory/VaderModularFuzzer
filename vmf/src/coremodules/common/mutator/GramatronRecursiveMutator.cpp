 ...
 */
StorageEntry* GramatronRecursiveMutator::createTestCase(StorageModule& storage, StorageEntry* baseEntry)
{
    ...
    recursive(newEntry,buffer, storage);
 ...
 */
void GramatronRecursiveMutator::random(StorageEntry* newEntry, char* buffer)
{
 ...
 * @param storage storage access need for benchmarking
 */
void GramatronRecursiveMutator::recursive(StorageEntry* newEntry, char* buffer, StorageModule& storage)
{
    Array* input = read_input(pda->state_ptr(),buffer);
    ...
    if(recurlen > 0) {
        ...
        GramatronRecursiveMutator::random(newEntry,buffer);
    }
