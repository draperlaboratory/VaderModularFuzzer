...
float AFLFavoredFeedback::computeFitness(StorageModule& storage, StorageEntry* e)
{
    ...
    if(favoredFitnessWeight > 1.0)
    {
      ...
      if(!topRatedInitialized)
      {
          //Map size is not known until the executor is set
...
{
  bool isFavored = false;
  char* coverage = executor -> getCoverageBits();
  int executionTime = executor -> getExecutionTime();
  long id = e -> getID();
  ...
  for (long removedID : unfavoredTestcases)
  {
    ...
    if (unfavorite)
    {
      ...
      StorageEntry * removedEntry = storage.getEntryByID(removedID);
      if (removedEntry != nullptr)
      {
        storage.unTagEntry(removedEntry, favoredTag);
        // Reduce fitness
  ...
  /*if (favoredBytesMarked > 0)
  {
    LOG_DEBUG << "New favored testcase " << id << " claimed " << favoredBytesMarked << " coverage bytes";
  }*/
