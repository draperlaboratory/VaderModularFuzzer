...
void AFLFeedback::registerMetadataNeeds(StorageRegistry& registry)
{
    ...
    bytesCoveredMetadata = registry.registerKey("TOTAL_BYTES_COVERED", StorageRegistry::INT, StorageRegistry::WRITE_ONLY);
    mapSizeMetadata = registry.registerKey("MAP_SIZE", StorageRegistry::INT, StorageRegistry::WRITE_ONLY);    
}   

void AFLFeedback::setExecutor(ExecutorModule* executor)
{
    this->executor = AFLExecutor::castTo(executor);

    // After configuring executor, we know mapsize
    mapSize = this->executor->getCoverageSize();
}

bool AFLFeedback::evaluateTestCaseResults(StorageModule& storage, StorageEntry* e)
{
    StorageEntry& metadata = storage.getMetadata();

    //First check the return status and increment crashed and hung metrics
    int exitStatus = executor->getReturnStatus();
    if(AFLExecutor::AFL_STATUS_CRASHED == exitStatus)
    {
        metadata.incrementIntValue(crashedTotalMetadata);
    }
    else if (AFLExecutor::AFL_STATUS_HUNG == exitStatus)
    {
        metadata.incrementIntValue(hungTotalMetadata);
    }

    //Compute average metrics
    int execTime = executor->getExecutionTime();
    ...
    bool shouldBeSaved = executor->hasNewCoverageBits();
    if(shouldBeSaved)
    {
        //Compute the fitness
        ...
        //Add tags for anything that crashed or hung
        if(AFLExecutor::AFL_STATUS_CRASHED == exitStatus)
        {
        ...
        {
            storage.tagEntry(e, hungTag);
        }
        else
        {
            storage.tagEntry(e, normalTag);
        }

        metadata.setValue(bytesCoveredMetadata,executor->getCorpusCoverageByteCount());
        metadata.setValue(mapSizeMetadata,mapSize);
    }
...
{
    int coverage = executor->getCoverageByteCount();
    int execTime = executor->getExecutionTime();
    int size = e->getBufferSize(testCaseKey);
