...
protected:
  AFLMutatorTest() {
    ...
    registry = new StorageRegistry("TEST_INT", StorageRegistry::INT,
                                   StorageRegistry::ASCENDING);
  ...
  void SetUp() override {
    ...
    int_key = registry->registerKey("TEST_INT", StorageRegistry::INT,
                                    StorageRegistry::READ_WRITE);
    normalTag =
        registry->registerTag("RAN_SUCCESSFULLY", StorageRegistry::WRITE_ONLY);
  ...
  // test algorithms that are intended to change buffer contents but preserve
  // buffer length
  void testAlgBuffEqLength(MutatorModule &mutator, StorageEntry *baseEntry) {
    ...
    GTEST_COUT << "Base: " << baseEntry->getBufferPointer(testCaseKey)
               << "\tMod: " << modEntry->getBufferPointer(testCaseKey) << "\n";

    EXPECT_EQ(baseEntry->getBufferSize(testCaseKey),
              modEntry->getBufferSize(testCaseKey))
        << name << " buffer size not equal";
    EXPECT_TRUE(areBuffersDifferent(baseEntry->getBufferPointer(testCaseKey),
                                    modEntry->getBufferPointer(testCaseKey),
                                    baseEntry->getBufferSize(testCaseKey)))
        << name << " buffers are not different.";
  }
  ...
  void testSplice(int baseSize, int secondSize) {
    ...
    // each test case is entirely populated single value, which is different for
    // each case and cannot be 1 which is used for the base case
    StorageEntry *entry;
    char *buf;
    for (int i = 2; i <= 6; i++) {
    ...
    // first index should be the same as baseCase (baseCase should not be
    // entirely overwritten)
    EXPECT_EQ(baseVal, newBuf[0])
        << "First index of baseCase has been overwritten";

    // test that everything after the splice point comes from the second
    // testcase in the splice
    ...
    for (int p = 0; p < newBufSize; p++) {
      if (false == foundSplice) {
        ...
        EXPECT_EQ(splicedVal, newBuf[p])
            << "Values after splice point not all the same (i.e. from second "
               "test case)";
      }
    }
    if (newBufSize > 1) {
      EXPECT_TRUE(foundSplice) << "Spliced test case does not appear to "
                                  "contain values from 2 different test cases";
    }
    // If the resulting buffer is only 1 character long, then there is no second
    // byte
  }

  void test_length_preserving_mutations(MutatorModule *mutator) {
    for (int i = 0; i < 10000; i++) {
      ...
      mutator->init(*config);
      testAlgBuffEqLength(*mutator, baseEntry);
    }
    delete mutator;
  }

  void test_length_changing_mutations(MutatorModule* mutator) {
    for (int i = 0; i < 10000; i++) {
      // create a base entry for algorithms to modify
      StorageEntry *baseEntry = storage->createNewEntry();
      char *buff = baseEntry->allocateBuffer(testCaseKey, 12);
      ...
      // GTEST_COUT << "Base: " << baseEntry->getBufferPointer(testCaseKey) <<
      // "\tMod: " << modEntry->getBufferPointer(testCaseKey) << "\n";
      EXPECT_NE(baseEntry->getBufferSize(testCaseKey),
                modEntry->getBufferSize(testCaseKey))
          << "mutator did not change buffer size";
    }
    delete mutator;
  }
...
TEST_F(AFLMutatorTest, TestAlgorithms) {
  ...
  EXPECT_NE(baseEntry->getBufferSize(testCaseKey),
            modEntry->getBufferSize(testCaseKey))
      << "DELETE BYTES did not change buffer size";
  ...
  EXPECT_NE(baseEntry->getBufferSize(testCaseKey),
            modEntry2->getBufferSize(testCaseKey))
      << "CLONE BYTES did not change buffer size";
}

// 2 tests to have tear down clean up in between
TEST_F(AFLMutatorTest, TestSpliceAlgorithm) { testSplice(2, 5); }
TEST_F(AFLMutatorTest, TestSpliceAlgorithm2) { testSplice(100, 20); }
TEST_F(AFLMutatorTest, TestSpliceAlgorithm3) { testSplice(5, 2); }
TEST_F(AFLMutatorTest, TestSpliceAlgorithm4) { testSplice(20, 100); }
TEST_F(AFLMutatorTest, TestSpliceAlgorithm_Short1) { testSplice(1, 10); }
TEST_F(AFLMutatorTest, TestSpliceAlgorithm_Short2) { testSplice(10, 1); }

TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlipBitMutator) {
  test_length_preserving_mutations(
      new AFLFlipBitMutator("AFLFlipBitMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlip2BitMutator) {
  test_length_preserving_mutations(
      new AFLFlip2BitMutator("AFLFlip2BitMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlip4BitMutator) {
  test_length_preserving_mutations(
      new AFLFlip4BitMutator("AFLFlip4BitMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlipByteMutator) {
  test_length_preserving_mutations(
      new AFLFlipByteMutator("AFLFlipByteMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlip2ByteMutator) {
  test_length_preserving_mutations(
      new AFLFlip2ByteMutator("AFLFlip2ByteMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLFlip4ByteMutator) {
  test_length_preserving_mutations(
      new AFLFlip4ByteMutator("AFLFlip4ByteMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLRandomByteAddSubMutator) {
  test_length_preserving_mutations(
      new AFLRandomByteAddSubMutator("AFLRandomByteAddSubMutator"));
}
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLRandomByteMutator) {
  test_length_preserving_mutations(
      new AFLRandomByteMutator("AFLRandomByteMutator"));
}

// In the case of a test buffer larger than 1 byte, the delete mutator should
// always produce a novel test case
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLDeleteMutator) {
  test_length_changing_mutations(
      new AFLDeleteMutator("AFLDeleteMutator"));
}

// In the case of a test buffer that is 1 byte, the delete mutator should always
// the same test case
TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLDeleteMutator1Byte) {
  for (int i = 0; i < 10000; i++) {
    // create a base entry for algorithms to modify
    StorageEntry *baseEntry = storage->createNewEntry();
    ...
    EXPECT_EQ(baseEntry->getBufferSize(testCaseKey),
              modEntry->getBufferSize(testCaseKey))
        << "DELETE BYTES did changed 1-byte buffer";
  }
}

TEST_F(AFLMutatorTest, ConsistentlyDifferentAFLCloneMutator) {
  test_length_changing_mutations(
      new AFLCloneMutator("AFLCloneMutator"));
}